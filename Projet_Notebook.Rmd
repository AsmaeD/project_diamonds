...
__Remarque__ : Notre jeu de données étant très volumineux, nous allons étudier qu'un échantillon représentant 10% de ce jeu de données.
...
On retrouve bien les 3 variables qualitatives __cut__, __color__ et __clarity__ avec leurs différentes modalités. Toutes les autres variables sont des variables quantitatives.
...
Pour ce qui est des variables quantitatives, on remarque que l'étendu de la variable __carat__ est assez grande. Cela veut surement dire qu'il y a des diamants très différents dans ce jeu de données.
...
Aux vues des valaurs que peuvent prendre les variables x, y et z, on peut simplement dire que les diamants de ce jeu de données ont tendances à être plus larges et profonds que longs.
...
Pour la clareté, il est surement très rare de trouver des diamants ayant la meilleure clareté (I1) et encore une fois il est possible qu'il soit coutûme de ne pas mettre en vente de diamants ayant une qualité de clareté médiocre si celui-ci ne possède pas de caractéristiques intéressantes par ailleurs.
...
On remarque que la majorité des diamants de ce jeu de données ont des prix inférieurs à 5000 dollars (on retrouve les résultats du summary) et que ceux ayant un prix supérieur à 12000 dollars sont considérés comme outliers par le logiciel Rstudio.
...
Aux vues des résultats, il semble que 31.8 et 58.9 soientt des erreurs de recopie et que les bonnes valeurs soient plutôt 3.18 et 5.89. Nous décidons donc de modifier ces valeurs.
...
On retrouve la même répartition des données pour la variable __cut__, on a toujours en majorité des diamants ayant une coupe de qulité supérieure ou égale à Very Good.

On retrouve également une répartition similaire pour la variable __clarity__. En revanche, la répartition de la variable __color__ est encore plus gaussienne que sur l'échantillon filtré que sur l'échantillon complet, avec la modalité G la plus représentée.
...
Les groupes les mieux représentés dans le plan F1/F2 sont les groupes "weight", "size" et "size_frequencies" qui regroupent toutes les variables quantitatives. Le groupe "physical_aspect" n'est lui pas bien représenté dans ce plan, nous ne pourrons donc pas dire grand chose sur ce groupe.
...
fviz_contrib(data_mfa, choice="group", axes=1:2)
```
...
Il en ressort que les variables qualitatives sont légèrement corrélés au différents variables quantitatives, ce qui ne va pas nous arranger car les variables quantitatives sont déjà très corrélées entre elles.
...
Ce grap,hique des variables quantitatives nous donne le cercle des corrélation des variables, celles-ci étant colorées en fonction de leur groupe d'appartenance.
...
Les 2 variables du groupe "size_frequencies" sont très positivement corrélées à l'axe 2.
...
Les individus se trouvant aux alentours du centre du grpahique semblent très mal représentés sur le plan F1/F2. En revanche ceux étant plus éloignés sont mieux représentés. Ils le sont encore plus quand ils sont du coté positif du premier axe.
...
```{r}
plot.PCA(data_acp,choix="ind",select="cos2 0.8",unselect=0.8, habillage='cos2')
```
...
Conclusion : 
Avec cette Analyse sur Composantes Principales on pourrait donc conclure que pour expliquer le prix d'un diamant, il suffit de connaitre sa longueur, largeur, profondeur et poids. Et que plus un diamant est lourd, long, large et profond, plus son prix sera élevé.
--> A finir
...
Dans une régression linéaire multiple, la variable à expliquer Y est une variable quantitative et toutes les variables explicatives le sont également. C'est pourquoi nous excluons les variables qualitatives de cette analyse.
...
## __6.1 Vérification des Postulats__
...
On remarque grâce au test vif que nos variables sont très corrélées entre elles (on le savait déjà grâce à l'ACP effectuée précédemment), cela va donc sûrement poser problème pour la régression multiple qui supporte très mal les soucis de multicorélation.
...
La p-value nous indique le rejet de l'hypoyhèse nulle d'indépendance des résidus. De manière générale l'indépendance des résidus est supposée vraie de par la façon dont les données ont été collectées. Or ici, nous n'avons pas accès à un document explicatif sur la façon dont cette base de données a été créée. Il se peut très bien que tous les diamants proviennent d'un même bijoutier auquel cas, effectivement les données sont biaisées.
...
Nous allons maintenant effectuer des régressions différentes de la régression multiple en ce que celles ci peuvent régler les problèmes de multicorélation des variables.
...
Pour l'axe 2, on retrouve une forte corrélation positive avec la variable __table__  et une cette fois on a une forte corrélation négative avec la variable __fdepth__ qui avait été placée en supplémentaire lors de l'ACP. L'axe 2 met donc en opposition les diamands ayant une forte valeur de __table__ et une faible valeur de __fdpeth__ à ceux ayant au contraire une faible valeur de __table__ mais une valeur de __fdepth__ élevée.

Enfin, l'axe 3 met lui en opposotion les diamants ayant de forte valeurs de __fdepth__ et __table__ à ceux ayant des faibles valeurs pour les deux variables.
...
On voit grâce à ce graphique que la prédiction faite par notre modèle n'est pas très satisfaisante. La présence d'outliers et de prix prédits négatifs montre que le modèle peut surement être amélioré. Il semble aussi que le modèle estime bien les prix des diamands vendus entre 1000 et 10000 dollars mais beacoup moins bien les autres fourchettes de prix. 
...
Le prix d'un diamant est donc fortement positivement lié à sa largeur, sa longeur, son poids et sa profondeur. Par exemple, si l'on augmente d'une unité la variable carat, le log du prix du diamant augmente 0.24, toutes choses égales par ailleurs.
...
Cette méthode (PCR) nous a donc permis de trouver un modèle satisfaisant pour expliquer le log du prix des diamants de la base de données. Cependant, la régression sur composantes princiaples est une méthode maximisant uniquement la variance des X (variables explicatives). Nous allons donc pour finir notre étude, utiliser une autre méthode de régression sur variables latentes, la PLS.
